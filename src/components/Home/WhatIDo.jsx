      ...
      <motion.div ref={ref} initial="hidden" animate={isInView ? 'visible' : 'hidden'} variants={containerVariants}>
        ...
        <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mt-10" variants={containerVariants}>
          {services.slice(0, 5).map((service, index) => (
            ...
            >
              <div>
                <img src={serviceImages[index]} alt={service.title} className="rounded-2xl w-[70px] h-auto" />
              </div>
              <div>
                <h4 className="text-2xl sm:text-3xl font-semibold">{service.title}</h4>
                <p className="text-gray-600 text-base sm:text-lg md:text-xl mt-3 md:mt-5">{service.description}</p>
                <button
                  onClick={() => handleLearnMore(service.slug)}
                  className="text-[#FF7537] font-semibold text-base sm:text-lg md:text-xl mt-5 hover:text-[#022F2B] transition-colors"
                >
                  {service.buttonText}
                </button>
              </div>
          ...
          >
            <div>
              ...
              <h4 className="text-2xl sm:text-3xl font-semibold text-white">Have project?</h4>
            </div>
            <div>
              <button
